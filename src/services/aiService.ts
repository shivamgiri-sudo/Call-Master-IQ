...
export interface SlideContent { title: string; bullets: string[]; }
export interface PptxContent { slides: SlideContent[]; }
export interface SheetData { sheetName: string; headers: string[]; rows: (string | number)[][]; }
export interface XlsxContent { sheets: SheetData[]; }
export interface DocSection { heading: string; paragraphs: string[]; }
export interface DocxContent { title: string; sections: DocSection[]; }
export type PdfContent = DocxContent;
export type DocumentContent = PptxContent | XlsxContent | DocxContent | PdfContent;
...
export async function generateDocumentContent(prompt: string, type: DocumentType): Promise<DocumentContent> {
