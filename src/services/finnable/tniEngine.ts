/**
 * Finnable Intelligence — TNI (Training Need Identification) Engine
 * Ported from finnable-dashboard/src/engine/tniEngine.js
 * Pure functions — no DB access, no side effects.
 */
import { TNIItem, ProcessTNIItem, ParameterHeatmapItem, TNIPriority } from './types';

const PARAM_NAMES = ['Opening', 'Discovery', 'Pitch', 'Journey', 'Objection', 'Compliance', 'Closing'];

function parseParameterScores(context: string): Record<string, { score: number | null; max: number | null; pct: number | null }> {
  const result: Record<string, { score: number | null; max: number | null; pct: number | null }> = {};
...
export function buildParameterHeatmap(rows: any[]): ParameterHeatmapItem[] {
  const params: Record<string, { name: string; total: number; scored: number; sumPct: number; low: number; zero: number }> = {};
  (rows || []).forEach(r => {
...
export function tniPriority(fatalCount: number, defectCount: number): TNIPriority {
...
export function buildTNI(rows: any[]): TNIItem[] {
...
export function buildProcessTNI(tniRows: TNIItem[]): ProcessTNIItem[] {
  const grouped: Record<string, {
    category: string;
    subCategory: string;
    defectCount: number;
