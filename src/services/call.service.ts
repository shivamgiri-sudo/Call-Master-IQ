...
export async function listCalls(user: AuthUser | undefined, filters: any) {
  ...
  const params: any[] = [startDate, endDate, ...scope.params];
  let f = '';
...
export async function getCallDetail(sourceType: string, callId: string) {
...
export function buildHeuristicInsight(d: any) {
  ...
  return {
    ...
    best_response:
      'Acknowledge, probe exact issue, confirm ownership, give clear timeline, and close with commitment.',
    evidence_moments: [
...
export async function getCachedInsight(sourceType: string, callId: string) {
  const [rows]: any = await db.query(
    `SELECT * FROM call_ai_insight
