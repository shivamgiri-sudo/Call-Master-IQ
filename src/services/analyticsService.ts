...
function buildFilter(f: AnalyticsFilter) {
  const conditions: string[] = [];
  const params: any[] = [];

  const { clause: scopeClause, params: scopeParams } = buildScopeWhereClause(f.scope);
  if (scopeClause !== '1=1') { conditions.push(scopeClause); params.push(...scopeParams); }

  if (f.client_id) { conditions.push('client_id = ?'); params.push(f.client_id); }
  if (f.from) { conditions.push('call_date >= ?'); params.push(f.from); }
  if (f.to) { conditions.push('call_date <= ?'); params.push(f.to); }
  if (f.source_type) { conditions.push('source_type = ?'); params.push(f.source_type); }
  if (f.process_name) { conditions.push('process_name = ?'); params.push(f.process_name); }
  if (f.business_lob) { conditions.push('business_lob = ?'); params.push(f.business_lob); }
  if (f.branch_short_name) { conditions.push('branch_short_name = ?'); params.push(f.branch_short_name); }

  return { where: conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '', params };
}

export async function getDashboardKPIs(f: AnalyticsFilter) {
...
export async function getAgentPerformance(f: AnalyticsFilter) {
  const { where, params } = buildFilter(f);
...
export async function getBranchPerformance(f: AnalyticsFilter) {
  const { where, params } = buildFilter(f);
...
export async function getProcessPerformance(f: AnalyticsFilter) {
  const { where, params } = buildFilter(f);
...
export async function getQualityTrends(f: AnalyticsFilter, granularity: 'daily' | 'weekly' = 'daily') {
  const { where, params } = buildFilter(f);
...
export async function getParameterFailureRates(f: AnalyticsFilter) {
  const { where, params } = buildFilter(f);
  ...
  const [rows] = await pool.execute<any[]>(
    `SELECT
     ...
     FROM v_call_master_inbound ${inboundWhere}`,
    params
  );
...
export async function getDailyReport(f: AnalyticsFilter) {
  const { where, params } = buildFilter(f);

  const [byBranch] = await pool.execute<any[]>(
    `SELECT branch_short_name, source_type,
       COUNT(*) AS total_calls,
       ROUND(AVG(quality_score), 2) AS avg_quality_score,
       COUNT(CASE WHEN quality_score >= 80 THEN 1 END) AS good_plus,
       COUNT(CASE WHEN quality_score < 50 THEN 1 END) AS miss_count,
     ...
     ORDER BY branch_short_name`,
    params
  );

  const [byProcess] = await pool.execute<any[]>(
    `SELECT process_name, business_lob, source_type,
       COUNT(*) AS total_calls,
       ROUND(AVG(quality_score), 2) AS avg_quality_score,
       COUNT(CASE WHEN quality_score >= 80 THEN 1 END) AS good_plus,
       COUNT(CASE WHEN quality_score < 50 THEN 1 END) AS miss_count,
