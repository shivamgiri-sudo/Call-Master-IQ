...
export async function createManualAudit(data: {
...
export async function updateManualAudit(audit_id: number, data: {
  scores?: Record<string, number | null>;
  manual_remarks?: string;
  audit_status?: 'draft' | 'submitted';
}) {
  ...
  if (data.manual_remarks !== undefined) { updates.push('manual_remarks = ?'); params.push(data.manual_remarks); }
  if (data.audit_status) { updates.push('audit_status = ?'); params.push(data.audit_status); }

  if (updates.length === 0) return;

  params.push(audit_id);
  await pool.execute(`UPDATE manual_qa_audit SET ${updates.join(', ')} WHERE audit_id = ?`, params);
}

export async function getManualAudits(filters: {
  auditor_user_id?: number;
  client_id?: string;
  agent_employee_code?: string;
  audit_status?: string;
  ...
  if (filters.auditor_user_id) { conditions.push('auditor_user_id = ?'); params.push(filters.auditor_user_id); }
  if (filters.client_id) { conditions.push('client_id = ?'); params.push(filters.client_id); }
  if (filters.agent_employee_code) { conditions.push('agent_employee_code = ?'); params.push(filters.agent_employee_code); }
  if (filters.audit_status) { conditions.push('audit_status = ?'); params.push(filters.audit_status); }
  if (filters.from) { conditions.push('call_date >= ?'); params.push(filters.from); }
...
export async function getManualAuditById(audit_id: number) {
  const [rows] = await pool.execute<any[]>(
    'SELECT * FROM manual_qa_audit WHERE audit_id = ?', [audit_id]
  );
  return rows[0] || null;
}

export async function runAIReaudit(transcript: string, promptConfig: {
