...
export async function generateCoachingContent(input: {
...
export async function getCoachingList(filters: {
  client_id?: string;
  business_lob?: string;
  defect_parameter?: string;
  ...
  if (filters.business_lob) { conditions.push('business_lob = ?'); params.push(filters.business_lob); }
  if (filters.defect_parameter) { conditions.push('defect_parameter = ?'); params.push(filters.defect_parameter); }
...
export async function getCoachingById(coaching_id: number) {
  const [rows] = await pool.execute<any[]>(
    'SELECT * FROM coaching_content WHERE coaching_id = ?', [coaching_id]
  );
  return rows[0] || null;
}

export async function assignCoaching(coaching_id: number, employee_codes: string[], assigned_by: number) {
  for (const code of employee_codes) {
    await pool.execute(
      `INSERT INTO coaching_assignment (coaching_id, employee_code, assigned_by_user_id)
       VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE completion_status = 'pending', assigned_at = NOW()`,
      [coaching_id, code, assigned_by]
    );
  }
}
...
export async function updateAssignmentStatus(assignment_id: number, status: 'viewed' | 'completed') {
  await pool.execute(
    `UPDATE coaching_assignment
