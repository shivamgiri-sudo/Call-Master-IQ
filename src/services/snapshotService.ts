...
export async function computeDailySnapshot(date: string) {
  // Delete any existing snapshot for this date first (idempotent)
  await pool.execute(`DELETE FROM daily_performance_snapshot WHERE snapshot_date = ?`, [date]);
  ...
  await pool.execute(
    `INSERT INTO daily_performance_snapshot
     ...
     ON DUPLICATE KEY UPDATE
       ...
       critical_calls = critical_calls + VALUES(critical_calls)`,
    [date, date]
  );
...
async function generateQualityDipAlerts(date: string) {
  const [snapshots] = await pool.execute<any[]>(
    `SELECT client_id, process_name, branch_short_name, source_type,
            avg_quality_score AS today_score
     FROM daily_performance_snapshot
     WHERE snapshot_date = ? AND avg_quality_score IS NOT NULL`,
    [date]
  );

  for (const snap of snapshots) {
    const [prev] = await pool.execute<any[]>(
      `SELECT AVG(avg_quality_score) AS prev_avg
       FROM daily_performance_snapshot
       WHERE snapshot_date >= DATE_SUB(?, INTERVAL 7 DAY)
         AND snapshot_date < ?
         AND client_id = ? AND process_name = ?
         AND branch_short_name = ? AND source_type = ?`,
      [date, date, snap.client_id, snap.process_name, snap.branch_short_name, snap.source_type]
    );
...
export async function runSnapshotForYesterday() {
