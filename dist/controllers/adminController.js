...
async function listPrompts(req, res) {
    try {
        ...
        const [rows] = await db_1.default.execute(`SELECT prompt_id, client_id, process_name, business_lob, source_type,
              prompt_version, is_active, created_by, created_at, updated_at
       FROM audit_prompt_config ${where} ORDER BY client_id, process_name, source_type`, params);
...
async function upsertPrompt(req, res) {
    try {
        if (!req.user) {
        ...
        await db_1.default.execute(`UPDATE audit_prompt_config SET is_active = 0
       WHERE client_id = ? AND source_type = ?
         AND (process_name <=> ?) AND (business_lob <=> ?)`, [client_id, source_type, process_name || null, business_lob || null]);
        const [result] = await db_1.default.execute(`INSERT INTO audit_prompt_config (client_id, process_name, business_lob, source_type, system_prompt, created_by)
       VALUES (?, ?, ?, ?, ?, ?)`, [client_id, process_name || null, business_lob || null, source_type, system_prompt, req.user.login_id]);
...
async function listEmployees(req, res) {
    try {
        const conditions = ['active_status = 1'];
        const params = [];
        if (req.query.branch) {
        ...
        const [rows] = await db_1.default.execute(`SELECT employee_id, employee_code, employee_name, designation, location,
              cost_center, branch_name, branch_short_name, active_status
       FROM employee_mapping_master ${where}
       ORDER BY employee_name LIMIT 500`, params);
        res.json({ success: true, data: rows });
    }
    catch (err) {
        res.status(500).json({ success: false, message: err.message });
...
async function importEmployees(req, res) {
    try {
        ...
        for (const emp of employees) {
            const [existing] = await db_1.default.execute('SELECT employee_id FROM employee_mapping_master WHERE employee_code = ?', [emp.employee_code]);
            if (existing.length > 0) {
                await db_1.default.execute(`UPDATE employee_mapping_master
           SET employee_name = ?, designation = ?, location = ?, cost_center = ?,
               branch_name = ?, branch_short_name = ?, updated_by = ?, updated_at = NOW()
           WHERE employee_code = ?`, [emp.employee_name, emp.designation || null, emp.location || null,
                    emp.cost_center || null, emp.branch_name || null, emp.branch_short_name || null,
                    req.user.login_id, emp.employee_code]);
                updated++;
            }
            else {
                await db_1.default.execute(`INSERT INTO employee_mapping_master
             (employee_code, employee_name, designation, location, cost_center, branch_name, branch_short_name, created_by)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [emp.employee_code, emp.employee_name, emp.designation || null, emp.location || null,
...
async function listProcesses(req, res) {
    try {
        const [rows] = await db_1.default.execute(`SELECT * FROM process_mapping_master WHERE active_status = 1 ORDER BY process_name`);
        res.json({ success: true, data: rows });
    }
    catch (err) {
        res.status(500).json({ success: false, message: err.message });
...
async function upsertProcess(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ success: false, message: 'Unauthorized' });
            return;
        ...
        if (mapping_id) {
            await db_1.default.execute(`UPDATE process_mapping_master
         SET dialdesk_client_id = ?, process_name = ?, business_lob = ?,
             source_type = ?, branch = ?, updated_by = ?, updated_at = NOW()
         WHERE mapping_id = ?`, [dialdesk_client_id, process_name, business_lob, source_type, branch, req.user.login_id, mapping_id]);
            res.json({ success: true, message: 'Process mapping updated' });
        }
        else {
            const [result] = await db_1.default.execute(`INSERT INTO process_mapping_master
           (dialdesk_client_id, process_name, business_lob, source_type, branch, created_by)
         VALUES (?, ?, ?, ?, ?, ?)`, [dialdesk_client_id, process_name, business_lob, source_type, branch, req.user.login_id]);
...
async function triggerSnapshot(req, res) {
    try {
        const { computeDailySnapshot } = await Promise.resolve().then(() => __importStar(require('../services/snapshotService')));
