...
async function getDashboardSummary(user, startDate, endDate) {
    ...
    const [summaryRows] = await db_1.db.query(`SELECT
     ...
     WHERE call_datetime >= ? AND call_datetime < ? ${scope.whereSql}`, params);
    const [branchRows] = await db_1.db.query(`SELECT
     ...
     LIMIT 100`, params);
    const [trendRows] = await db_1.db.query(`SELECT
...
function buildRoleInsight(roleCode) {
    const map = {
        CEO: [
